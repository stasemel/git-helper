...
- **git add .** лобавить все из текущей папки
...
- **git push -u origin master** отправить изменения первый раз ветки master
- **git push -u origin <имя ветки>** отправить изменения ветки
...
- **git checkout -b <название_ветки>** создать ветку и сразу на неё переключиться
- **git branch -a** отобразить все ветки, в том числе git-hub
- **git diff <название_ветки1> <название_ветки2>** сравнить ветки
- **git diff <название_ветки1> <хеш>** сравнить ветки и коммиты
- **\~** суффикс навигации коммитов. Нумерация начинается с нуля: commit~0 — это сам коммит, commit~1 — предыдущий, commit~2 — предшествующий предыдущему и так далее. Например, HEAD~1 — это следующий за текущим коммит. А main~5 — это пятый коммит в ветке main, если считать с последнего выполненного коммита.
- **git merge <название_ветки>** слияние ветокю перед вызовом надо перейти в ту, в которую сливается изменение, в main, например
- **git branch -D <название_ветки>** удалить ветку после объединения. Перед удалением надо перейти в другйо бранч.  Удаление локальной ветки через Git не удаляет ветку на GitHub!
- **git branch -d <название_ветки>** безопасно удалить ветку после объединения
